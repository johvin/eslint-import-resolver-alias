...
- The `map` item `['helper', './utils/helper']` means that the modules' paths which contain the word `helper` will be resolved to new paths (e.g. `helper => ./utils/helper`, `helper/datefn => ./utils/helper/datefn`) which are located relative to the `process current working directory` (almost the project root directory). See [issue #3](https://github.com/johvin/eslint-import-resolver-alias/issues/3)
- The order of 'material-ui/DatePicker' and 'material-ui' cannot be reversed, otherwise the alias rule 'material-ui/DatePicker' does not work. See [issue #7](https://github.com/johvin/eslint-import-resolver-alias/issues/7)
